---
title: "Lingering land use legacies after different wetland restoration strategies"
...
Project Description: Anthropogenic legacy effects often occur as a consequence of land use change and can leave behind long-lasting changes to ecosystem structure and function. These physical, chemical, and biological changes can linger and interact with contemporary management to impact restoration outcomes. In the present study, we evaluated how restoration strategy can overcome the lingering effects of land-use in wetlands. 
...
toc <- boxplot(WL.soil.nuts$TOC ~ WL.trts, 
  ...
  axis(side = 2, labels = T, tck = -0.02, lwd = 1.5, las = 1)
  axis(side = 4, labels = F, tck = -0.02, lwd = 1.5)
  axis(side = 2, labels = F, tck = 0.01, lwd = 1.5)
  axis(side = 4, labels = F, tck = 0.01, lwd = 1.5)
  box(lwd = 1.5)
...
# Simple Community Hypothesis Testing
```{r}
...
WL.unifrac.dist2 <- as.matrix(WL.unifrac.dist)[setdiff(labels(WL.unifrac.dist),
                              odd.sites), setdiff(labels(WL.unifrac.dist),
                              odd.sites)]

WL.unifrac.adonis <- adonis(as.dist(WL.unifrac.dist2) ~ treatments.2, perm = 999)
WL.unifrac.adonis
...
# Which plants influenced plant communities between trts
...
# Plant Communities
library("labdsv")
plant.ind <- indval(WL.plant, treatments)
summary(plant.ind)
plant.ind$indval
levels(treatments)

plant.groups <- rep(NA, length(treatments))
for (i in 1:length(treatments)){
  if (treatments[i] == "Reference" | 
      treatments[i] == "Bareroot"){
    plant.groups[i] <- "Group 1"
  }
  if (treatments[i] == "Seedling" | 
      treatments[i] == "BalledBurlapped"){
  plant.groups[i] <- "Group 2"
  }
  if (treatments[i] == "Seedbank" |
      treatments[i]== "Acorn"){
  plant.groups[i] <- "Group 3"
  }
}

plant.ind2 <- indval(WL.plant, plant.groups)
summary(plant.ind2)

WLdataREL.2 <- WLdataREL.2[, colSums(WLdataREL.2) > 0]
bac.ind <- indval(WLdataREL.2, treatments.2)
summary(bac.ind)

#bac.ind

WLdataREL.2 <- WLdataREL.2[, colSums(WLdataREL.2) > 0.05]
bac.ind <- indval(WLdataREL.2, treatments.2)
levels(treatments.2)
summary(bac.ind)
...
ind.tax <- WL.tax[which(as.character(WL.tax$OTU) %in% bac.indicators$OTU), ]
...
# Define Plot Parameters
par(mar = c(5, 6, 2, 1), oma = c(2, 1, 1, 1))
...
arrows(y0 = cent.dataframe[,2], 
       ...
       length=0.1, lwd = 2, code = 3)
points(cent.dataframe[,1], cent.dataframe[,2], 
       cex = 5, bg = myColors, col = "black", pch = 22, lwd = 2)

range.x <- par("usr")[2] - par("usr")[1]
range.y <- par("usr")[4] - par("usr")[3]
text(par("usr")[1] + 0.1 * range.x, par("usr")[4] - 0.05 * range.y, 
     "Plant", cex = 1.75)

axis(side = 1, labels = T, las = 1, lwd.ticks = 2, cex.axis = 1.5)
axis(side = 2, labels = T, las = 1, lwd.ticks = 2, cex.axis = 1.5)
axis(side=1, lwd.ticks = 2, tck = -0.02, labels=F, cex.axis=1)
axis(side=3, lwd.ticks = 2, tck = -0.02, labels=F, cex.axis=1)
axis(side=1, lwd.ticks = 2, tck = 0.01, labels=F, cex.axis=1)
axis(side=3, lwd.ticks = 2, tck = 0.01, labels=F, cex.axis=1)
...
mtext(paste("PCoA 2 (", explainvar2, "%)", sep = ""), side = 2, 
      line = 4, cex = 1.5)
#mtext("Plant", side = 3, cex = 1.5, line = 1)
...
range.x <- par("usr")[2] - par("usr")[1]
range.y <- par("usr")[4] - par("usr")[3]
text(par("usr")[1] + 0.15 * range.x, par("usr")[4] - 0.05 * range.y, 
     "Bacteria", cex = 1.75)
#text(cent.dataframe[,1], cent.dataframe[,2], rownames(cent.dataframe))
...
mtext(paste("PCoA 2 (", explainvar2, "%)", sep = ""), side = 2, 
      line = 4, cex = 1.5)
#mtext("Bacteria", side = 3, cex = 1.5, line = 1)
...
legend("top", legend = c("Burlapped", cent.treats[-1]), 
       pt.bg = myColors, col = "black",
       pch = 22, cex = 1.5, bty = 'n', ncol = 3, pt.cex = 2)
...
mantel.rtest(dist.bac, dist.plant, nrepet = 999)
mantel.rtest(dist.bac, dist.nuts, nrepet = 999)
mantel.rtest(dist.bac, dist.phys, nrepet = 999)
mantel.rtest(dist.bac, dist.soil, nrepet = 999)
mantel.rtest(dist.plant, dist.nuts, nrepet = 999)
mantel.rtest(dist.plant, dist.phys, nrepet = 999)
mantel.rtest(dist.plant, dist.soil, nrepet = 999)
...
```{r}
# Nutrients
chem.dist <- dist(apply(WL.soil.nuts, 2, scale), 
                  method = "euclidean")
...
# Physical Factors
phys.dist <- dist(apply(WL.soil.phys, 2, scale), 
                  method = "euclidean")
...
# Bac dbRDA
dbRDA.c <- dbrda(bac.comm ~ chem.pcoa$points[,1], distance = "bray") 
anova(dbRDA.c, by = "terms", model = "direct") 
RsquareAdj(dbRDA.c)


dbRDA.p <- dbrda(bac.comm ~ phys.pcoa$points[,1], distance = "bray")
anova(dbRDA.p)
RsquareAdj(dbRDA.p)

dbRDA.cp <- dbrda(bac.comm ~ chem.pcoa$points[,1] + phys.pcoa$points[, 1], 
               distance = "bray", add = T)
anova(dbRDA.cp, by = "terms", model = "direct")
RsquareAdj(dbRDA.cp)


dbRDA.P.cp <- dbrda(plant.comm ~ phys.pcoa$points[,1] * chem.pcoa$points[,1], 
               distance = "bray", add = T)
anova(dbRDA.P.cp, by = "terms", model = "direct") 
RsquareAdj(dbRDA.P.cp)
```

# Microbial and Plant Bray Curtis Ordination (PCoA)
```{r, message=FALSE}
png(filename="../figures/WL.PCoA_Env.png",
    width = 800, height = 1800, res = 96*2)
...
WL.soil.nuts <- WL.soil2[, which(colnames(WL.soil2) %in% 
                                   c("TOC", "TN", "OM", "NH4.N", "NO3.N"))]
WL.soil.all <- WL.soil2[, which(colnames(WL.soil2) %in% 
                              c("Moisture", "Temp", "pH", "TOC", 
                                "OM", "NH4.N", "NO3.N"))]
fit <- envfit(WLplant_pcoa, WL.soil.all)
...
par(lwd = 2)
#plot(fit, p.max = 0.01, lwd = 1.5)
plot(fit, p.max = 0.01, labels = c("Moisutre", "Temp", "pH", "TOC", "OM", expression("NH"[4]), expression("NO"[3])), cex = 1.5)
par(lwd = 1)
...
WL.soil.all <- WL.soil2[, which(colnames(WL.soil2) %in% 
                              c("Moisture", "Temp", "pH", 
                                "OM", "NH4.N", "NO3.N"))]
fit <- envfit(WL_pcoa, WL.soil.all)
...
par(lwd = 2)
#plot(fit, p.max = 0.01)
plot(fit, p.max = 0.01, labels = c("Moisutre", "Temp", "pH", "OM", expression("NH"[4]), expression("NO"[3])), cex = 1.5)
par(lwd = 1)
...
## Show Plot
#```{r, echo=FALSE, fig.width = 2, fig.height = 2}
img <- readPNG("../figures/WL.PcoA_Env.png")
grid.raster(img)
```

# PostHoc Tests
```{r}
phys.mod <- lm(phys.pcoa$points[,1] ~ WL.soil2$Treatment)
chem.mod <- lm(chem.pcoa$points[,1] ~ WL.soil2$Treatment)
anova(phys.mod)
anova(chem.mod)
