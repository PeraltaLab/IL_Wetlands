...
WL.unifrac <- WL.unifrac.raw[which(row.names(WL.unifrac.raw) %in% row.names(WLdata.in)), which(row.names(WL.unifrac.raw) %in% row.names(WLdata.in))]
...
legend(0, 1, legend = c("Balled Burlapped", cent.treats[-1]), pt.bg = myColors, col = "black", 
...
grep("[0-9][0-9]$", rownames(WL.soil2)) == grep("S*$", rownames(bac.comm))
...
dbRDA.cp <- dbrda(WLdataREL ~ phys.pcoa$points[,1] * chem.pcoa$points[,1], 
               distance = "bray", add = T)
...
tuk <- HSD.test(lm(WL.soil.RCGbiomass ~ WL.soil.Treatment, 
                   data = WL.rc), "WL.soil.Treatment")
grp <- tuk$groups[c(match(gsub(" ", "", tuk$groups$trt), levels(WL.rc$WL.soil.Treatment))), ]
