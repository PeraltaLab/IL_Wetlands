...
WL.unifrac <- WL.unifrac.dist.raw[which(row.names(WL.unifrac.raw) %in% row.names(WLdata)), which(row.names(WL.unifrac.raw) %in% row.names(WLdata))]
...
# PERMANOVA
WL.bac.adonis <- adonis(WLdataREL ~ treatments, method = "bray", perm=99)
WL.bac.adonis

WL.unifrac.adonis <- adonis(WL.unifrac.dist ~ treatments, perm = 99)
WL.unifrac.adonis

WL.plant.adonis <- adonis(WL.plant ~ treatments, method = "bray", perm=99)
WL.plant.adonis
...
WL.unifrac.2 <- WL.unifrac[which(row.names(WL.unifrac) != odd.sites),
                           which(row.names(WL.unifrac) != odd.sites)]
WL.unifrac.dist2 <- as.dist(WL.unifrac.2, upper = T, diag = T)

treatments <- design[which(row.names(design) != odd.sites), ]$Treatment
...
png(filename="../figures/WL.plant.PCoA.png",
    width = 1200, height = 800, res = 96*2)
# Create Distance Matrix
WL.plant2 <- WL.plant[which(design$Treatment != "Bareroot"), ]
treatments <- design$Treatment[design$Treatment != "Bareroot"]
samplePlant.dist <- vegdist(WL.plant2, method="bray")
...
anova(dbRDA) # How to pull out the percent variation explained...not sure
...
```{r}
myColors <- c("#FFF000", "#CCFF00", "#33CC33", "#000000", "#336633", "#FF9933") #pick new
names(myColors) <- levels(WL.rc$WL.soil.Treatment)
colScale <- scale_colour_manual(values = myColors)
...
text(x = seq_along(levels(WL.rc$WL.soil.Treatment)), y = par("usr")[3] - 15, srt = 45, adj = 1, labels = levels(WL.rc$WL.soil.Treatment), xpd = TRUE)
