...
WL.bac.adonis <- adonis(WLdataREL.2 ~ treatments.2, method = "bray", perm=99)
WL.bac.adonis
...
#WL.bac.simper <- simper(WLdataREL, group = treatments, permutations = 99)
#bac.sum <- summary(WL.bac.simper, ordered = T)

WL.plant.simper <- simper(WL.plant, group = treatments, permutations = 99)
...
dbRDA <- dbrda(WLdataREL ~ phys.pcoa$points[,1] + chem.pcoa$points[,1], distance = "bray")
anova(dbRDA) 
str(dbRDA)
...
dbRDA <- dbrda(WL.plant ~ phys.pcoa$points[,1] + chem.pcoa$points[,1], distance = "bray")
anova(dbRDA) 
